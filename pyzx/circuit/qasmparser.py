    ...
    def parse_command(self, c: str, registers: Dict[str,Tuple[int,int]]) -> List[Gate]:
        ...
        for j in range(dim):
            ...
            if name in self.customgates:
                ...
                continue
            if name == "swap":
                g1 = qasm_gate_table["cx"](control=argset[0],target=argset[1])
                g2 = qasm_gate_table["cx"](control=argset[1],target=argset[0])
                g3 = qasm_gate_table["cx"](control=argset[0],target=argset[1])
                for gate in [g1,g2,g3]:
                    gates.append(gate)
                continue            
            if name in ("x", "z", "s", "t", "h", "sdg", "tdg", "y"):
                if name == "y": #Y = S X S^\dagger
                    g1 = qasm_gate_table["s"](argset[0])
                    g2 = qasm_gate_table["x"](argset[0])
                    g3 = qasm_gate_table["s"](argset[0],adjoint=True)
                    for gate in [g1,g2,g3]:
                        gates.append(gate)
                    continue
                if name in ("sdg", "tdg"): 
                ...
                gates.append(g)
                continue
            if name.startswith("rx") or name.startswith("rz") or name.startswith("ry") or name.startswith("u1") or name.startswith('crz'):
                i = name.find('(')
                j = name.find(')')
                ...
                elif name.startswith('ry'): #Ry = S Rx S^\dagger
                    g1 = qasm_gate_table["s"](argset[0])
                    g2 = XPhase(argset[0],phase=phase)
                    g3 = qasm_gate_table["s"](argset[0], adjoint=True)
                    for gate in [g1,g2,g3]:
                        gates.append(gate)
                    continue                
